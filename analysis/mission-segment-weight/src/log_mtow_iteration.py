...
#  Modifies the called json file to reflect the new mtow
#
...
with open(log_path, "w", newline="") as csvfile:
    ...
    csvwriter.writerow([
        ...
        "empty_mass_kg", "fuel_mass_kg", "payload_mass_kg", "total_fuel_consumed"
    ])
    for row in history:
        csvwriter.writerow([
            row["iteration"],
            ...
            f"{row['fuel_mass_kg']:.6f}",
            f"{row['payload_mass_kg']:.6f}",
            f"{row['total_fuel_consumed']:.6f}"

        ])

json_path = os.path.join(log_dir, cfg)
...
data['aircraft']["max_takeoff_mass_kg"] = final_mtow
